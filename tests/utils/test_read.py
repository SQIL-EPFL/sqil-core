...
class TestExtractH5Data:
    ...
    def test_if_invalid_key_return_empty(self, temp_h5_file):
        arr1,  test1, test2, test3 = extract_h5_data(temp_h5_file, ['array1', '', True, None])
        assert np.array_equal(arr1, np.array(self.test_data['array1']))
...
class TestReadParamDict:
    ...
    "current": 0.001,
    "CW_onetone": {
        "ro_freq_start": 7526200000.0,
        "sweep": False
    },
    "CW_twotone": {
        "qu_freq_step": 250000.0
    },
    "vna_avg": 4,
    "sweep_start": -70,
    "sweep_list": [
    ...
    def test_reads_param_dict_into_a_dict_of_objects_with_metadata(self, temp_json_file):
        res = read_param_dict(temp_json_file)
        for param in res:
            assert res[param] == self.expected[param], f'got {str(res[param])} but should be {str(self.expected[param])}'
    ...
    def test_should_return_default_values_if_param_is_unknown(self, temp_json_file):
        ...
        assert str(res['test_id']) == '{"id": "test_id", "value": 51, "name": "test_id", "symbol": "test_id", "unit": "", "scale": 1}'
